    ...
    @if($page && $page->builder_content)
        @foreach($page->builder_content as $block)
                    ...
                    <section class="py-20 bg-[#1c1c1c]" id="destacados">
                        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                            <div class="text-center mb-16">
                                <h2 class="text-4xl md:text-5xl font-['Anton'] text-white uppercase tracking-widest mb-4 inline-block border-b-4 border-[#18833b] pb-2">
                                    {{ $block['data']['heading'] ?? 'Platillos Destacados' }}
                                </h2>
                                <p class="text-gray-400">{{ $block['data']['subtitle'] ?? 'Descubre lo mejor de nuestra cocina' }}</p>
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                                @foreach($featuredProducts as $product)
                                    <div class="group bg-gradient-to-b from-[#2a2a2a] to-[#1a1a1a] rounded-xl overflow-hidden shadow-2xl border border-gray-800 hover:border-gray-600 transition-all duration-300 transform hover:-translate-y-2">
                                        ...
                                        <div class="p-6">
                                            <div class="text-xs text-[#18833b] uppercase tracking-wider font-bold mb-2">
                                                {{ $product->category?->name ?? 'Especial' }}
                                            </div>
                                            <h3 class="text-2xl font-['Anton'] tracking-wider text-white mb-3">{{ $product->name }}</h3>
                    ...
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        class="relative w-full max-w-5xl mx-auto group pb-10">
                            ...
                            @if(count($block['data']['items'] ?? []) > 0)
                                <!-- Slides container -->
                                <div class="overflow-hidden rounded-2xl border-2 border-gray-800 shadow-2xl relative bg-[#1c1c1c]">
                                            ...
                                            <div class="w-full shrink-0 relative aspect-[21/9] group/slide cursor-pointer">
                                                ...
                                                @if(!empty($item['title']))
                                                    <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex items-end p-8">
                                                        <h3 class="text-3xl font-bold font-['Anton'] tracking-wider text-white uppercase drop-shadow-lg">{{ $item['title'] }}</h3>
                                                    </div>
                                                @endif
                                            </div>
                                        @endforeach
                                ...
                                @if(count($block['data']['items']) > 1)
                                    <!-- Controls -->
                                    <button @click="prev" class="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/60 hover:bg-[#E52B2B] text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all backdrop-blur z-30 shadow-xl border border-gray-700">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M15 19l-7-7 7-7"></path></svg>
                                    </button>
                                    <button @click="next" class="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/60 hover:bg-[#E52B2B] text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all backdrop-blur z-30 shadow-xl border border-gray-700">
                                    ...
                                    <div class="absolute pt-4 text-center w-full flex justify-center space-x-3">
                                        <template x-for="i in slides">
                                            <button @click="activeSlide = i - 1"
                                                    ...
                                                    :class="activeSlide === i - 1 ? 'bg-[#E52B2B] w-3 h-3' : 'bg-gray-600 hover:bg-gray-400'"></button>
                                        </template>
                                    </div>
                                @endif
                            @endif
                ...
                <section class="py-24 bg-[#111111] relative overflow-hidden" id="about">
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                        <div class="flex flex-col lg:flex-row items-center gap-16">
                            <div class="lg:w-1/2">
                                <h2 class="text-4xl md:text-5xl font-['Anton'] text-white uppercase tracking-widest mb-6">
                                    {{ $block['data']['heading'] ?? 'Historia de Asados' }}
                                </h2>
                                <div class="prose prose-invert prose-lg text-gray-400">
                                    <p class="mb-6 whitespace-pre-line">{{ $block['data']['description'] ?? 'Pasión por el humo.' }}</p>
                                </div>
                                <div class="mt-8 flex gap-4">
                                    <div class="bg-black p-4 text-center rounded border border-gray-800">
                                        <svg class="w-8 h-8 mx-auto mb-2 text-[#E52B2B]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                        ...
                                        <span class="text-xs uppercase tracking-wider font-bold text-gray-500">Ahumado Lento</span>
                                    </div>
                                    <div class="bg-black p-4 text-center rounded border border-gray-800">
                                        <svg class="w-8 h-8 mx-auto mb-2 text-[#E52B2B]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
                                        </svg>
                                        <span class="text-xs uppercase tracking-wider font-bold text-gray-500">Cortes Premium</span>
                                    </div>
                                    <div class="bg-black p-4 text-center rounded border border-gray-800">
                                        <svg class="w-8 h-8 mx-auto mb-2 text-[#E52B2B]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                        </svg>
                                        <span class="text-xs uppercase tracking-wider font-bold text-gray-500">Estilo Texas</span>
                                    </div>
                                </div>
                            </div>
                            <div class="lg:w-1/2 relative">
                                <div class="absolute -inset-4 bg-[#E52B2B] rounded-xl transform rotate-3 opacity-20 blur-lg"></div>
                    ...
                    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="text-center mb-16" x-data="{ shown: false }" x-intersect.once="shown = true"
                            ...
                            <div class="inline-flex items-center gap-3 mb-4">
                                ...
                                <span class="text-[#E52B2B] uppercase tracking-[0.3em] text-sm font-bold">BBQ &amp; Smoke</span>
                                <div class="h-px w-12 bg-[#E52B2B]"></div>
                            </div>
                            <h2 class="text-4xl md:text-6xl font-['Anton'] text-white uppercase tracking-widest mb-4 drop-shadow-2xl">
                                {{ $block['data']['heading'] ?? 'Nuestros Ahumados' }}
                            </h2>
                            <p class="text-[#E52B2B] text-xl font-semibold italic mb-4">{{ $block['data']['subheading'] ?? 'Fuego lento. Sabor eterno.' }}</p>
                            <div class="h-1 w-24 bg-[#E52B2B] mx-auto rounded mb-6"></div>
                            @if(!empty($block['data']['description']))
                                <p class="text-gray-400 max-w-2xl mx-auto text-lg whitespace-pre-line">{{ $block['data']['description'] }}</p>
                            @endif
                        </div>
                                ...
                                @foreach($block['data']['items'] as $i => $item)
                                    <div x-data="{ shown: false }" x-intersect.once="shown = true"
                                        ...
                                        <div class="relative h-52 overflow-hidden">
                                            ...
                                            @if(!empty($item['badge']))
                                                <span class="absolute top-3 right-3 bg-[#E52B2B] text-white text-xs font-bold uppercase px-3 py-1 rounded-full shadow-lg">
                                                    {{ $item['badge'] }}
                                                </span>
                                            @endif
                                        </div>
                                        {{-- Info --}}
                                        <div class="p-5">
                                            <h3 class="text-white font-['Anton'] tracking-wider text-xl mb-1 uppercase">{{ $item['name'] ?? '' }}</h3>
                        ...
                        @else
                            ...
                            <div class="text-center py-12">
                                <p class="text-gray-600 italic">Configura los platillos BBQ desde el panel de administración.</p>
                            </div>
                        @endif
                    </div>
                </section>
                    ...
                    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-16"
                            ...
                            <div>
                                ...
                                <h2 class="text-4xl md:text-6xl font-['Anton'] text-white uppercase tracking-widest mb-3 drop-shadow-2xl">
                                ...
                                @if(!empty($block['data']['description']))
                                    <p class="text-gray-400 mt-4 max-w-xl text-base whitespace-pre-line">{{ $block['data']['description'] }}</p>
                                @endif
                            </div>
                            {{-- Horario badge --}}
                            <div class="shrink-0">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                                @foreach($block['data']['tacos'] as $i => $taco)
                                    <div x-data="{ shown: false }" x-intersect.once="shown = true"
                                        ...
                                        {{-- Info --}}
                                        <div class="p-5">
                                            <h3 class="text-white font-['Anton'] tracking-wider text-xl mb-1 uppercase">{{ $taco['name'] ?? '' }}</h3>
                        ...
                        @else
                            <div class="text-center py-12">
                                <p class="text-gray-600 italic">Configura los tacos y platillos nocturnos desde el panel de administración.</p>
                            </div>
                        @endif
                    </div>
                </section>
                ...
                <section class="py-24 bg-[#1c1c1c] relative overflow-hidden" id="ahumados">
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div class="text-center mb-16" x-data="{ shown: false }" x-intersect.once="shown = true"
                            ...
                            <p class="text-gray-400 max-w-3xl mx-auto text-lg whitespace-pre-line">{{ $block['data']['description'] ?? '' }}</p>
                        </div>
                ...
                <section class="py-20 bg-black relative" id="reviews">
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div class="text-center mb-16">
                            ...
                            <p class="text-gray-500 mt-4 text-sm tracking-widest uppercase">Reseñas verificadas de nuestros clientes</p>
                        </div>

                        @if($reviews->isEmpty())
                            <p class="text-center text-gray-600 text-sm">Aún no hay reseñas disponibles.</p>
                        @else
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            @foreach($reviews as $review)
                                <div x-data="{ shown: false }" x-intersect.once="shown = true"
                                    class="bg-[#1c1c1c] p-8 rounded-xl border border-gray-800 relative transition-all duration-1000 transform flex flex-col"
                                    :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'">

                                    {{-- Comillas decorativas --}}
                                    <span class="text-6xl text-[#E52B2B] font-serif absolute top-4 right-4 opacity-20 leading-none">"</span>

                                    {{-- Estrellas --}}
                                    <div class="flex items-center gap-1 mb-4">
                                        @for($i = 1; $i <= 5; $i++)
                                            <svg class="w-5 h-5 fill-current {{ $i <= $review->rating ? 'text-[#f97316]' : 'text-gray-700' }}" viewBox="0 0 20 20">
                                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                            </svg>
                                        @endfor
                                        <span class="ml-1 text-xs text-gray-500 font-medium">{{ $review->rating }}/5</span>
                                    </div>

                                    {{-- Comentario --}}
                                             <p class="text-gray-300 italic mb-6 flex-1 leading-relaxed">"{{ $review->comment ?? 'Excelente experiencia.' }}"</p>
                    ...
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div class="flex flex-col lg:flex-row gap-12 items-center">
                            <div class="lg:w-1/3 transition-all duration-1000 transform" x-data="{ shown: false }"
                                ...
                                <h2 class="text-4xl md:text-5xl font-['Anton'] text-white uppercase tracking-widest mb-6 border-l-4 border-[#E52B2B] pl-4">
                                    {{ $block['data']['heading'] ?? 'Encuéntranos' }}
                                </h2>
                                @if (!empty($block['data']['description']))
                                <p class="text-gray-400 mb-8 text-lg">{{ $block['data']['description'] }}</p>
                                @endif
                            ...
                            <div class="lg:w-2/3 w-full h-96 rounded-xl overflow-hidden shadow-2xl border-4 border-gray-800 transition-all duration-1000 transform"
                                x-data="{ shown: false }" x-intersect.once="shown = true"
                                :class="shown ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'">
            ...
            @if($block['type'] === 'contact_form')
                <section class="py-24 bg-[#111111] relative overflow-hidden" id="contacto-form">
                    ...
                    <div class="absolute top-0 left-1/2 -translate-x-1/2 w-96 h-48 bg-[#E52B2B]/5 blur-3xl rounded-full"></div>
                    ...
                    <div class="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div class="text-center mb-12" x-data="{ shown: false }" x-intersect.once="shown = true"
                            ...
                            <div class="inline-flex items-center gap-3 mb-4">
                                <div class="h-px w-12 bg-[#E52B2B]"></div>
                                <span class="text-[#E52B2B] uppercase tracking-[0.3em] text-sm font-bold">✉️ Contacto</span>
                                <div class="h-px w-12 bg-[#E52B2B]"></div>
                            </div>
                            <h2 class="text-4xl md:text-5xl font-['Anton'] text-white uppercase tracking-widest mb-4 drop-shadow-xl">
                                {{ $block['data']['heading'] ?? 'Contáctanos' }}
                            </h2>
                            <div class="h-1 w-20 bg-[#E52B2B] mx-auto rounded mb-4"></div>
                            @if(!empty($block['data']['description']))
                                <p class="text-gray-400 text-lg max-w-xl mx-auto">{{ $block['data']['description'] }}</p>
                            @endif
                        </div>
                ...
                <section class="py-24 bg-[#1c1c1c]">
                    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                        @if (!empty($block['data']['heading']))
                            <div class="text-center mb-12">
                                <h2 class="text-4xl md:text-5xl font-['Anton'] text-white uppercase tracking-widest mb-4">{{ $block['data']['heading'] }}</h2>
                    ...
                    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-{{ count($block['data']['plans'] ?? []) > 2 ? '3' : '2' }} gap-8">
                            @foreach ($block['data']['plans'] ?? [] as $plan)
                                ...
                                <div class="relative flex flex-col rounded-2xl border p-6 sm:p-8 transition-transform hover:-translate-y-1 duration-300 {{ $hl ? 'border-[#E52B2B] bg-[#1a1a1a] shadow-2xl sm:scale-105' : 'border-gray-700 bg-[#161616]' }}">
                                    @if ($hl)
                                        <span class="absolute -top-3 left-1/2 -translate-x-1/2 text-xs font-bold uppercase tracking-widest px-4 py-1 rounded-full text-white" style="background:{{ $accent }}">Recomendado</span>
                                    @endif
                                    <h3 class="text-xl font-bold text-white mb-2">{{ $plan['name'] ?? '' }}</h3>
                                    <div class="text-4xl font-['Anton'] mb-4" style="color:{{ $accent }}">{{ $plan['price'] ?? '' }}</div>
                                    @if (!empty($plan['description']))<p class="text-gray-400 text-sm mb-6">{{ $plan['description'] }}</p>@endif
            ...
            @if($block['type'] === 'team')
                <section class="py-24 bg-[#1c1c1c]">
                    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        @endif
                        @if (!empty($block['data']['intro']))<p class="text-gray-400 text-center mb-12">{{ $block['data']['intro'] }}</p>@endif
                            ...
                            @foreach ($block['data']['members'] ?? [] as $member)
                                <div class="bg-[#161616] border border-gray-800 rounded-2xl overflow-hidden hover:border-[#E52B2B] transition-all duration-300 group">
                                    @if (!empty($member['photo']))
                                        <div class="aspect-square overflow-hidden"><img src="{{ asset('storage/' . $member['photo']) }}" alt="{{ $member['name'] ?? '' }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"></div>
                                    @else
                                        <div class="aspect-square bg-[#222] flex items-center justify-center"><svg class="w-20 h-20 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></div>
                                    @endif
                                    <div class="p-6">
                                        ...
                                        @if (!empty($member['bio']))<p class="text-gray-400 text-sm mb-4">{{ $member['bio'] }}</p>@endif
                                    </div>
                                </div>
                            @endforeach
                        </div>
                            ...
                            @foreach ($block['data']['items'] ?? [] as $t)
                                <div class="bg-[#161616] border border-gray-800 rounded-2xl p-6 flex flex-col gap-4 hover:border-[#E52B2B] transition-all duration-300">
                                    ...
                                    <div class="flex items-center gap-3 mt-auto pt-4 border-t border-gray-800">
                                        ...
                                        @else
                                            <div class="w-10 h-10 rounded-full bg-[#E52B2B] flex items-center justify-center text-white font-bold text-sm">{{ strtoupper(substr($t['author'] ?? '?', 0, 1)) }}</div>
                                        @endif
                                        <div>
                                            <p class="text-white font-semibold text-sm">{{ $t['author'] ?? '' }}</p>
                                            @if (!empty($t['role']))<p class="text-gray-500 text-xs">{{ $t['role'] }}</p>@endif
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        @if ($lightbox)
                            <div x-data="{ lightboxOpen: false, lightboxSrc: '' }">
                                <div class="grid grid-cols-2 md:grid-cols-{{ $gcols }} gap-3">
                                    @foreach ($block['data']['images'] ?? [] as $img)
                                        <div class="overflow-hidden rounded-xl cursor-pointer group" @click="lightboxSrc = '{{ asset('storage/' . $img['src']) }}'; lightboxOpen = true">
                                            ...
                                            @if (!empty($img['caption']))<p class="text-center text-gray-500 text-xs mt-1">{{ $img['caption'] }}</p>@endif
                                        </div>
                                    @endforeach
                                </div>
                                <div x-show="lightboxOpen" x-cloak class="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" @click.self="lightboxOpen = false" @keydown.escape.window="lightboxOpen = false">
                                    <button @click="lightboxOpen = false" class="absolute top-4 right-4 text-white/70 hover:text-white"><svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
            ...
            @if($block['type'] === 'video_embed')
                ...
                <section class="py-24 bg-[#1c1c1c]">
                    <div class="{{ !empty($block['data']['full_width']) ? 'max-w-6xl' : 'max-w-3xl' }} mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        @endif
                        @if (!empty($block['data']['description']))<p class="text-gray-400 text-center mb-8">{{ $block['data']['description'] }}</p>@endif
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-{{ min(4, count($block['data']['items'] ?? [])) }} gap-8 text-center">
                            @foreach ($block['data']['items'] ?? [] as $stat)
                                ...
                                <div x-data="{ shown: false }" x-intersect.once="shown = true"
                                    ...
                                    <div class="text-gray-400 font-medium uppercase tracking-wider text-sm">{{ $stat['label'] ?? '' }}</div>
                                </div>
                            @endforeach
                        </div>
                    </div>
            ...
            @if($block['type'] === 'rich_text')
                ...
                <section class="py-24 bg-[#1c1c1c]">
                    <div class="{{ $mw }} mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        @endif
                        <div class="privacy-content text-gray-300">{!! $block['data']['content'] ?? '' !!}</div>
                    </div>
                </section>
            @endif
                                ...
                                @foreach ($sfPosts as $post)
                                    ...
                                    <div class="flex flex-col bg-[#1a1a1a] rounded-2xl overflow-hidden border border-gray-800 hover:border-gray-600 transition-all duration-300 shadow-xl group" style="height:600px;">
                                        <div class="flex items-center gap-2 px-4 py-3 border-b border-gray-800">
                                            ...
                                            <a href="{{ $url }}" target="_blank" rel="noopener" class="ml-auto text-gray-600 hover:text-gray-400 transition-colors">
                                        ...
                                        <div class="flex-1 overflow-hidden w-full">
                                            @if ($platform === 'tiktok')
                                                ...
                                                @else
                                                    <a href="{{ $url }}" target="_blank" rel="noopener" class="flex items-center justify-center h-full text-gray-400 hover:text-white transition-colors text-sm">Ver en TikTok</a>
                                                @endif
                                            @elseif ($platform === 'instagram')
                                                ...
                                                @else
                                                    <a href="{{ $url }}" target="_blank" rel="noopener" class="flex items-center justify-center h-full text-gray-400 hover:text-white transition-colors text-sm">Ver en Instagram</a>
                                                @endif
                                            @elseif ($platform === 'youtube')
                                                ...
                                                @else
                                                    <a href="{{ $url }}" target="_blank" rel="noopener" class="flex items-center justify-center h-full text-gray-400 hover:text-white transition-colors text-sm">Ver en YouTube</a>
                                                @endif
                                            @elseif ($platform === 'facebook')
